...
//form with React and JS validation
...
//form with React Router Form
export function FormSignUp({ errors, onSetErrors,submitting, onSetSubmitting}) {
    const data = useActionData()
    return(
                                //la moment las asa, dar trebuie modificat ruta in APP -> register/sign-up  regiaster/sign-in
...
export const formSignUpAction = async ({request}) => {
    console.log(request)
    ...
    // console.log((Object.keys(data).length))
        ...
        let error = {}
            ...
            if(!data.email ){
                ...
                return {error: "You need to enter a valid email address"}
            }
    
            if(!data.firstName || !data.lastName ){
                return {error: "Name is required"}
            }else if(!data.firstName.match(name_pattern) || !data.lastName.match(name_pattern)){
            ...
            } else if (!data.password.trim().match(password_pattern)){
                return {error: "Password must contain at least one capital letter, one lowercase letter, numbers and symbols"}
            }
